...
function replaceImg() {
  ...
  for (var i = source.length - 1; i >= 0; i--) {
    if ((source[i].getAttribute('src')).charAt(0) == '$') {
      var rightNameSource = source[i].getAttribute('src').split('*');
      var imgWidth = rightNameSource[0].slice(1);
      var imgHeight = rightNameSource[1];
      var imgColor = (rightNameSource[2].split('.')[0]).split('#')[1];
      source[i].src = createCanvas(imgWidth, imgHeight, imgColor);
    }
  }
}
...
function replaceA() {
  ...
  for (var i = source.length - 1; i >= 0; i--) {
    if ((source[i].getAttribute('href')).charAt(0) == '$') {
      var rightNameSource = source[i].getAttribute('href').split('*');
      var imgWidth = rightNameSource[0].slice(1);
      var imgHeight = rightNameSource[1];
      var imgColor = (rightNameSource[2].split('.')[0]).split('#')[1];
      source[i].href = createCanvas(imgWidth, imgHeight, imgColor);
    }
  }
}
